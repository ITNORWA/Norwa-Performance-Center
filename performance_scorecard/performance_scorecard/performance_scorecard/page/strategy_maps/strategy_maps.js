frappe.pages['strategy-maps'].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Strategy Maps',
		single_column: true
	});

	page.set_primary_action('Refresh', function () {
		load_root_nodes();
	});

	frappe.require('/assets/performance_scorecard/css/strategy_maps.css');

	$(page.body).append(`
		<div class="strategy-map-tree">
			<div id="strategy-tree"></div>
		</div>
	`);

	load_root_nodes();

	const $tree = $(page.body).find('.strategy-map-tree').first();
	const schedule_scale = create_scale_scheduler($tree);
	$(window).on('resize', schedule_scale);
};

function load_root_nodes() {
	$('#strategy-tree').empty();
	frappe.call({
		method: "performance_scorecard.performance_scorecard.page.strategy_maps.strategy_maps.get_root_nodes",
		callback: function (r) {
			if (r.message) {
				const roots = r.message.map(node => build_node(node, { depth: 1 }));
				$('#strategy-tree').append(roots);
				const $tree = $('.strategy-map-tree').first();
				if ($tree.length) {
					create_scale_scheduler($tree)();
				}
			}
		}
	});
}

function build_node(node, context) {
	const typeClass = get_type_class(node.type);
	const colorClass = get_color_class(node.progress, node.end_date);
	const depth = context.depth || 1;
	const column = ((depth - 1) % 4) + 1;

	const $node = $(`
		<div class="tree-node ${colorClass} ${typeClass} column-${column} ${node.expandable ? 'has-children' : ''}"
			 data-id="${node.id}"
			 data-type="${node.type}"
			 data-expandable="${node.expandable}">
			<div class="tree-content">
				<div class="tree-pill">
					<div class="tree-title">${node.label}</div>
					<div class="tree-type">${node.type}</div>
					${node.meta ? `<div class="tree-meta">${node.meta}</div>` : ''}
					<div class="tree-progress">
						<div class="progress">
							<div class="progress-bar" style="width: ${node.progress || 0}%"></div>
						</div>
						<span>${Math.round(node.progress || 0)}%</span>
					</div>
				</div>
				${node.expandable ? '<div class="tree-expand">+</div>' : ''}
			</div>
			<div class="tree-children"></div>
		</div>
	`);

	$node.data('context', { ...context, depth: depth });

	$node.find('.tree-content').on('click', function (e) {
		e.stopPropagation();
		if (!$node.data('expandable')) return;

		if ($node.hasClass('expanded')) {
			$node.removeClass('expanded');
			$node.find('.tree-expand').first().text('+');
			return;
		}

		if ($node.find('.tree-children').children().length > 0) {
			$node.addClass('expanded');
			$node.find('.tree-expand').first().text('-');
			return;
		}

		load_children($node.data('type'), $node.data('id'), $node.data('context'), $node);
	});

	return $node;
}

function load_children(nodeType, nodeId, parentContext, $node) {
	let context = { ...parentContext };
	if (nodeType === 'Department') {
		context.department = nodeId;
	} else if (nodeType === 'Employee') {
		context.employee = nodeId;
	}
	context.depth = (parentContext.depth || 1) + 1;

	frappe.call({
		method: "performance_scorecard.performance_scorecard.page.strategy_maps.strategy_maps.get_children",
		args: {
			node_type: nodeType,
			node_id: nodeId,
			context: context
		},
		freeze: true,
		callback: function (r) {
			const $children = $node.find('.tree-children');
			$children.empty();

			if (r.message && r.message.length > 0) {
				r.message.forEach(child => {
					$children.append(build_node(child, context));
				});
				$node.addClass('expanded');
				$node.find('.tree-expand').first().text('-');
				const $tree = $node.closest('.strategy-map-tree');
				if ($tree.length) {
					create_scale_scheduler($tree)();
				}
			} else {
				frappe.show_alert('No further items found.');
				$node.find('.tree-expand').remove();
			}
		}
	});
}

function create_scale_scheduler($tree) {
	let timeout_id = null;
	return function () {
		if (timeout_id) {
			clearTimeout(timeout_id);
		}
		timeout_id = setTimeout(() => {
			update_tree_scale($tree);
		}, 60);
	};
}

function update_tree_scale($tree) {
	if (!$tree || !$tree.length) return;
	const container = $tree.get(0);
	const style = getComputedStyle(container);
	const node_width = parseFloat(style.getPropertyValue('--tree-node-width')) || 200;
	const node_gap = parseFloat(style.getPropertyValue('--tree-node-gap')) || 50;
	const padding = parseFloat(style.paddingLeft || 0) + parseFloat(style.paddingRight || 0);
	const available = Math.max(container.clientWidth - padding, 1);

	let max_depth = 1;
	$tree.find('.tree-node').each(function () {
		const depth = $(this).parents('.tree-node').length + 1;
		if (depth > max_depth) {
			max_depth = depth;
		}
	});

	const columns = max_depth;
	const needed = columns * (node_width + node_gap) + node_width;
	const scale = needed > 0 ? (available / needed) : 1;
	$tree.css('--tree-scale', scale.toString());
}

function get_color_class(progress, end_date) {
	progress = progress || 0;
	if (end_date) {
		let today = frappe.datetime.get_today();
		if (end_date < today && progress < 100) {
			return 'node-red';
		}
	}
	if (progress >= 100) return 'node-green';
	if (progress >= 76) return 'node-blue';
	if (progress >= 51) return 'node-yellow';
	return 'node-red';
}

function get_type_class(type) {
	if (!type) return "type-generic";
	const t = type.toLowerCase();
	if (t.includes("company")) return "type-company";
	if (t.includes("department")) return "type-department";
	if (t.includes("employee")) return "type-employee";
	if (t.includes("kpa")) return "type-kpa";
	if (t.includes("goal")) return "type-goal";
	if (t.includes("kra")) return "type-kra";
	if (t.includes("kpi")) return "type-kpi";
	return "type-generic";
}
