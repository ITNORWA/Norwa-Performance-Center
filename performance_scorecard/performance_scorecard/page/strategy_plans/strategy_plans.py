    ...
    for kpa, data in rollup.items():
        ...
        for goal_name, goal_data in data["goals"].items():
            ...
            for kra_name, kra_data in goal_data["kras"].items():
                ...
                kra_weight_val = kra_data.get("weightage") or 1

                kra_rows.append(
                    {
                        "kra": kra_name,
                        "average_score": kra_avg,
                        "weightage": kra_weight_val,
                    }
                )
                goal_sum += kra_avg * kra_weight_val
                goal_weight += kra_weight_val
...
def _build_company_rollups():
    ...
    for kpa, goals_list in kpa_groups.items():
        if not goals_list:
            continue
        goal_sum = sum((g.get("average_score") or 0) * ((g.get("weightage") or 1)) for g in goals_list)
        goal_weight = sum((g.get("weightage") or 1) for g in goals_list) or 1
        kpa_avg = goal_sum / goal_weight

        kpa_weight_val = kpa_weights.get(kpa) or 1
        overall_sum += kpa_avg * kpa_weight_val
        overall_weight += kpa_weight_val
...
def _apply_department_progress(goals_list, department):
    if not goals_list or not department:
        return

    dept_goal_names = [g.get("name") for g in goals_list if g.get("name")]
